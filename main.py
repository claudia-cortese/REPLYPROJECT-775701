...
1. **Width and Height**: the dimensions of the images can affect the input layer of our neural network. Uniform image sizes are typically required for most deep learning models. Large variations in size may necessitate resizing, which could affect the quality or aspect ratio of our images.
...
The first graph illustrates the distribution of image widths in the dataset. The histogram is sharply peaked around 760 pixels, indicating that most images have a width very close to this value. There is a small amount of variability, with a few images having widths slightly below 740 or slightly above 780 pixels. The narrow distribution suggests that the images are generally uniform in width, which can be beneficial for consistency in image processing tasks.

**Image Contrast Distribution**

The second graph shows the distribution of image contrast levels. The histogram reveals a roughly normal distribution, centered around a contrast value of approximately 60. The spread of the distribution indicates that while most images have moderate contrast, there is a reasonable range of contrast levels present in the dataset. This variation in contrast is important for ensuring that the model can handle images with different levels of contrast during training and inference.

**Image Brightness Distribution**

The third graph presents the distribution of image brightness. The histogram shows a distribution that is slightly skewed to the left, with a peak around a brightness value of 240. The majority of images are quite bright, with brightness values predominantly between 230 and 250. There are fewer images with lower brightness levels. This distribution suggests that the dataset consists mostly of bright images, which could influence the model's performance and its ability to generalize to darker images if they are underrepresented.
"""
...
We are going to start by introducing two feature extraction methods, in particular Local Binary Patterns and PCA
...
# Replace it with your desired directories
...
"""The silhouette score is a useful metric for evaluating the quality of clustering, but it is not the only criterion to consider. The score measures how similar each point is to its own cluster compared to other clusters, but it might not capture all the nuances of our specific data and clustering needs.
...
"""We tried implementeing DBSCAN as an alternative clustering algorithm. However, the results we obtained weren't satisfactory and therefore we decided to discard this method.

Here our comments on the results:
...
After just 10 epochs the model reports an accuracy of almost 98% on the test set. As amazing as this is, it's crucial to validate the model thoroughly to ensure its robustness and generalizability.
"""
...
# Evaluating the model on the test set
test_loss, test_accuracy = ann_model_2.evaluate(test_pca, y_test_pca, verbose=0)
print(f'Test Accuracy: {test_accuracy:.4f}')
...
# Evaluating the model on the test set
test_loss, test_accuracy = ann_model_3.evaluate(test_pca, y_test_pca, verbose=0)
print(f'Test Accuracy: {test_accuracy:.4f}')
print(f'Test Loss: {test_loss:.4f}')
...
"""Seemilarly to what we did for LBP and PCA, we are displaying examples from each cluster obtained with the encoded features.

By visually inspecting the clusters it's easy to visually recognize patterns.
"""

# Adding cluster labels to the original dataframe for encoded features
train_df['Cluster_Encoded'] = encoded_labels_train
test_df['Cluster_Encoded'] = encoded_labels_test

# Displaying examples from each cluster for encoded features
for cluster in range(4):
    print(f"\nCluster {cluster}:")
    cluster_samples = test_df[test_df['Cluster_Encoded'] == cluster].sample(5)
    for index, row in cluster_samples.iterrows():
        image_path = row['FilePath']
        with Image.open(image_path) as img:
            plt.imshow(img, cmap='gray')
            plt.title(f"Cluster {cluster} (Encoded): {row['FileName']}")
            plt.axis('off')
            plt.show()
...
labels = np.hstack((encoded_labels_train, encoded_labels_test))  # Using the labels from the best KMeans run
...
# Applying K-Means on PCA features
kmeans_pca = KMeans(n_clusters=4, random_state=42, n_init=10)
kmeans_pca.fit(train_pca)
...
"""In order to get insights into the classification performance of the ANN model on the VAE latent features, we are going to plot a confusion matrix showing the counts of correct and incorrect predictions for each class.

The confusion matrix shows that the ANN model performs well for Labels 2 and 3, with high correct classification rates and minimal misclassification. However, there is notable confusion between Labels 0 and 2, and Labels 1 and 3, indicating that these labels share similar features in the latent space. True Label 1 has a significant number of samples misclassified as Label 3, showing a high level of confusion between these two labels. True Label 0 also has a substantial number of samples misclassified as Label 2.

Overall, while the model is effective for some labels, there are areas where it struggles to distinguish between certain clusters. The PCA-based ANN Model showed much better results and is considered to be the most appropriate for our particular task.
"""

# Predicting the classes for the test data
y_pred_vae = ann_model_vae.predict(encoded_test)
y_pred_vae_classes = np.argmax(y_pred_vae, axis=1)
y_true_vae = np.argmax(y_test_vae, axis=1)

# Generating the confusion matrix
conf_matrix = confusion_matrix(y_true_vae, y_pred_vae_classes)
...
plt.xlabel('Predicted Label')
plt.ylabel('True Label')
plt.title('Confusion Matrix (ANN on VAE Latent Space)')
plt.show()
...
In summary, the PCA-based Model achieved the best results. Moreover, by displaying samples from each cluster we can visually observe a clear distinction between clusters as well as similarities in layout characteristics within each cluster.
"""